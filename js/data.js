...
function getFeaturedSlots(limit = 4) {
  // Pick evenly across providers so all are represented
  const byProvider = {};
  for (const s of SLOTS) {
    if (!s.featured) continue;
    (byProvider[s.provider_id] = byProvider[s.provider_id] || []).push(s);
  }
  const pools = Object.values(byProvider);
  const result = [];
  let i = 0;
  while (result.length < limit && pools.some(p => p.length > 0)) {
